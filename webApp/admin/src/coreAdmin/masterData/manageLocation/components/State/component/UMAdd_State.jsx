...
export default class UMAdd_State extends TrackerReact(Component) {
	...
	render(){
        ...
        return(
			<tr>
				<td className="rolelst"> {this.props.dataindex+1}</td>			
				<td className="rolelst"> {this.props.StateDataValues.stateName}</td>			
				<td className="roletbl"> 
					<button className="editrole fa fa-pencil-square-o editbtns editbtnshvr"  data-toggle="modal" data-target={`#editstate-${this.props.dataindex}`} ></button>						
					...
					<button className= "roleDelete editbtnred fa fa-trash delIcon detailsCenter editbtns btn-danger" data-toggle="modal" data-target={`#delstate-${this.props.dataindex}`}></button>
