...
function fetchPlanStatusCenter(center_ID,planYear){
            ...
            .then(centerList=>{
            	...
            	async function getData(){
            		...
            		for(j = 0 ; j < centerList.length ; j++){
	            		var monthlyData = await fetchPlanStatusCenter(centerList[j]._id,req.params.planYear);
	            		if(monthlyData){
                            request({
                                        "method"    : "GET", 
                                        "url"       : "http://localhost:"+globalVariable.port+"/api/masternotification/send-mail",
                                        "body"      : {
                                                            "templateName": "annualplanreminder",
                                                            "variables"   : {
