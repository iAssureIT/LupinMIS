...
// router.get('/activitywise_report/:startDate/:endDate/:center_ID/:sector_ID/:projectCategoryType/:projectName/:uidstatus/:activity_ID/:subactivity_ID' ,  Reports2Controller.activitywise_report);
    ...
    .then(data=>{
	    ...
	    async function getData(){
			...
			var benData = {"date": {$gte : req.params.startDate, $lte : req.params.endDate} };
	        	// console.log("req.params.center_ID============",req.params.center_ID)  	    
			if(req.params.center_ID !== 'all'){
				benData.center_ID = req.params.center_ID;
			}
	        for (var i = 0; i < data.length; i++) {
				benData.projectCategoryType = data[i].projectCategoryType;
				benData.projectName         = data[i].projectName;
				benData.sector_ID           = data[i].sector_ID;
				benData.activity_ID         = data[i].activity_ID;
				benData.subactivity_ID      = data[i].subactivity_ID;
				benData.unit                = data[i].unit;
	            ...
	            var activitydetails =  {              
	                ...
	                "unit"                   : data[i].unit,
	                "unitCost"               : data[i].total            ? ((data[i].total/data[i].quantity)/100000).toFixed(2) : 0,// Average unitCost calculated dividing total by quantity
	        ...
	        if(i >= data.length && data.length > 0){
	            activitydata.push( 
		            ...
		            {              
		                ...
		                "unitCost"               : "-",
		                "quantity"               : "-",
		                "total"                  : totalTotal > 0      ? "<b>"+((totalTotal / totalTotal)*100).toFixed(2) + "%" + "</b>": "<b>" + 0 + "</b>",
		                "LHWRF"                  : totalLHWRF > 0      ? "<b>"+((totalLHWRF / totalTotal)*100).toFixed(2) + "%" + "</b>": "<b>" + 0 + "</b>",
		                "NABARD"                 : totalNABARD > 0     ? "<b>"+((totalNABARD / totalTotal)*100).toFixed(2) + "%" + "</b>": "<b>" + 0 + "</b>",
		                "bankLoan"               : totalbankLoan > 0   ? "<b>"+((totalbankLoan / totalTotal)*100).toFixed(2) + "%" + "</b>": "<b>" + 0 + "</b>",
		                "directCC"               : totaldirectCC > 0   ? "<b>"+((totaldirectCC / totalTotal)*100).toFixed(2) + "%" + "</b>": "<b>" + 0 + "</b>",
		                "indirectCC"             : totalindirectCC > 0 ? "<b>"+((totalindirectCC / totalTotal)*100).toFixed(2) + "%" + "</b>": "<b>" + 0 + "</b>",
		                "govtscheme"             : totalgovtscheme > 0 ? "<b>"+((totalgovtscheme / totalTotal)*100).toFixed(2) + "%" + "</b>": "<b>" + 0 + "</b>",
		                "other"                  : totalother > 0      ? "<b>"+((totalother / totalTotal)*100).toFixed(2) + "%" + "</b>": "<b>" + 0 + "</b>",
		                "reach"                  : "",
		                "familyUpgradation"      : "",
		            }
	            );
...
function getBeneficiaryCount(query){
    ...
    return new Promise(function(resolve,reject){
	    ...
	    .then(actData=>{
			...
			// console.log('benficiariesArray',benficiariesArray.length);
			var flags = {};
...
function CenterAchievements_Report(selector1){
...
exports.plan_vs_Achievement_Physical = (req,res,next)=>{
    ...
    .then(data=>{
	    ...
	    async function getData(){
	        ...
	        // console.log("activitydata",activitydata)
	        for (var i = 0; i < data.length; i++) {
				selector1.sector_ID = (data[i]._id);
				selector2.sector_ID = (data[i]._id).toString();
    ...
    .then(data=>{
	    ...
	    async function getCenterData(){
	    	...
	    	for (var i = 0; i < data.length; i++) {
				...
				var centerAchievements = await CenterAchievements_Report(selector1)  
				var benCounts          = await getBeneficiaryCount(selector1)
    ...
    .then(data=>{
	    ...
	    async function getCenterData(){
	        ...
	        var totalFamilyUpgradation  = 0;
				var selector1 = {"date": {$gte : req.params.startDate, $lte : req.params.endDate} };
	    	for (var i = 0; i < data.length; i++) {
				selector1.center_ID    = data[i]._id.toString();
				var centerAchievements = await CenterAchievements_Report(selector1)  
				var benCounts          = await getBeneficiaryCount(selector1)
				// console.log('centerAchievements',centerAchievements);  		
				// console.log('benCounts',benCounts);  
...
exports.center_wise_Achievements = (req,res,next)=>{
    ...
    .then(data=>{
	    ...
	    async function getCenterData(){
			...
			var centerAchievements = await CenterAchievements_Report(selector1)  
			// console.log("2. time = ", new Date());

	    	for (var i = 0; i < data.length; i++) {
				selector1.center_ID = data[i]._id.toString();
				var benCounts          = await getBeneficiaryCount(selector1)
				// console.log('benCounts',i,benCounts);  
								
				const index = centerAchievements.findIndex((element) => {				  
					if(data[i]._id){
				    	return element.center_ID === data[i]._id.toString()
					}
				})
				...
				if(index !== -1){
					var centerdetails =  {              
		                "reach"                  : benCounts.reachCount                    ? (benCounts.reachCount) : 0,
		                "familyUpgradation"      : benCounts.upgradedfamilyCount           ? (benCounts.upgradedfamilyCount) : 0,
		                "center_ID"              : centerAchievements[index].center_ID     ? centerAchievements[index].center_ID     : data[i]._id,
		                "centerName"             : centerAchievements[index].centerName    ? centerAchievements[index].centerName    : data[i].centerName,
		                "total"                  : centerAchievements[index].total         ? Number(centerAchievements[index].total/100000).toFixed(2) : 0,
		                "LHWRF"                  : centerAchievements[index].LHWRF         ? Number(centerAchievements[index].LHWRF/100000).toFixed(2) : 0,
		                "NABARD"                 : centerAchievements[index].NABARD        ? Number(centerAchievements[index].NABARD/100000).toFixed(2) : 0,
		                "bankLoan"               : centerAchievements[index].bankLoan      ? Number(centerAchievements[index].bankLoan/100000).toFixed(2) : 0,
		                "directCC"               : centerAchievements[index].directCC      ? Number(centerAchievements[index].directCC/100000).toFixed(2) : 0,
		                "indirectCC"             : centerAchievements[index].indirectCC    ? Number(centerAchievements[index].indirectCC/100000).toFixed(2) : 0,
		                "govtscheme"             : centerAchievements[index].govtscheme    ? Number(centerAchievements[index].govtscheme/100000).toFixed(2) : 0,
		                "other"                  : centerAchievements[index].other         ? Number(centerAchievements[index].other/100000).toFixed(2) : 0,
		            }
				}else{
					var centerdetails =  {              
		                "reach"                 : benCounts.reachCount                ? (benCounts.reachCount) : 0,
		                "familyUpgradation"     : benCounts.upgradedfamilyCount       ? (benCounts.upgradedfamilyCount) : 0,
						"center_ID"           	: data[i]._id,
	    ...
	    async function getData(){
	    	...
	    	for (var i = 0; i < data.length; i++) {
				...
				var centerdetails =  {              
	                "reach"                  : benCounts.reachCount                    ? (benCounts.reachCount) : 0,
	                "familyUpgradation"      : benCounts.upgradedfamilyCount           ? (benCounts.upgradedfamilyCount) : 0,
	                "center_ID"              : activitydata[0].center_ID               ? activitydata[0].center_ID     : data[i]._id,
	                "centerName"             : activitydata[0].centerName              ? activitydata[0].centerName    : data[i].centerName,
	                "total"                  : activitydata[0].total                   ? Number(activitydata[0].total).toFixed(2)         : 0,
	                "LHWRF"                  : activitydata[0].LHWRF                   ? Number(activitydata[0].LHWRF).toFixed(2)         : 0,
	                "NABARD"                 : activitydata[0].NABARD                  ? Number(activitydata[0].NABARD).toFixed(2)        : 0,
	                "bankLoan"               : activitydata[0].bankLoan                ? Number(activitydata[0].bankLoan).toFixed(2)      : 0,
	                "directCC"               : activitydata[0].directCC                ? Number(activitydata[0].directCC).toFixed(2)      : 0,
	                "indirectCC"             : activitydata[0].indirectCC              ? Number(activitydata[0].indirectCC).toFixed(2)    : 0,
	                "govtscheme"             : activitydata[0].govtscheme              ? Number(activitydata[0].govtscheme).toFixed(2)    : 0,
	                "other"                  : activitydata[0].other                   ? Number(activitydata[0].other).toFixed(2)         : 0,
	            }
		        centerData.push(centerdetails);      
			}    
