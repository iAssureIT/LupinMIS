    ...
    .then(data=>{
         var allData = _.flatten(data.map((a, i)=>{
            ...
            return {
                ...
                "projectName"         : '<p  class="prewrapText text-left"><b>'+a.projectName+'</b></p><br>'+'<p class="text-left">'+moment(a.startDate).format('DD-MM-YYYY')+'</p>'+'<p class="text-left">'+moment(a.endDate).format('DD-MM-YYYY')+'</p>', 
                // "startDate"           : a.startDate, 
                // "endDate"             : a.endDate, 
                "sectorName"          : ((a.sector.map((b, j)=>{return '<p class="prewrapText text-left">'+b.sectorName+'</p>'})).toString()).replace(/,/g, " "),
    ...
    .then(data=>{
         var allData = _.flatten(data.map((a, i)=>{
            return {
                ...
                // "typeofGoal"          : a.typeofGoal,
                // "goalName"            : '<div class="wrapText text-left">'+a.goalName+'</div>',
                "projectName"         : '<p  class="prewrapText text-left"><b>'+a.projectName+'</b></p><br>'+'<p>'+moment(a.startDate).format('DD-MM-YYYY')+'</p>'+'<p>'+moment(a.endDate).format('DD-MM-YYYY')+'</p>', 
                // "startDate"           : a.startDate, 
                // "endDate"             : a.endDate, 
                "sectorName"          : ((a.sector.map((b, j)=>{return '<p class="prewrapText text-left">'+b.sectorName+'</p>'})).toString()).replace(/,/g, " "),
                "activityName"        : ((a.sector.map((b, j)=>{return '<p class="prewrapText text-left">'+b.activityName+'</p>'})).toString()).replace(/,/g, " ") ? ((a.sector.map((b, j)=>{return '<p class="prewrapText text-left">'+b.activityName+'</p>'})).toString()).replace(/,/g, " ") : "-",
...
exports.delete_projectMapping = (req,res,next)=>{   
    ProjectMapping.find({_id:req.params.projectMappingID})
    .exec()
    .then(data=>{
            ...
            .then(activityData=>{
                ...
                .then(plansData=>{
                    ...
                    if(activityData.length>0 && plansData.length>0){
                        res.status(200).json({
                            "message" : "Project Mapping can not be deleted, It is already used in Quarterly Plan & Activity Report"
                        });
                    }else if(activityData.length==0 && plansData.length>0){
                        res.status(200).json({
                            "message" : "Project Mapping can not be deleted, It is already used in Quarterly Plan"
                        });
                    }else if(activityData.length>0  && plansData.length==0){
                        res.status(200).json({
                            "message" : "Project Mapping can not be deleted, It is already used in Activity Report"
                        });
                    }else{    
